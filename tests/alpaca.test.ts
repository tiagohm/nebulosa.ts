...
test.skip('make image bytes from fits', async () => {
	...
	for (const bitpix of [8, 16]) {
		for (const channel of CHANNELS) {
			...
			if (channel === 1) {
				await saveAndCompareHash(jpeg.compress(bytes, 706, 1037, 'GRAY', 100, 'GRAY', output)!, `imagebytes-${bitpix}-1.jpg`, '', true)
			} else {
				await saveAndCompareHash(jpeg.compress(bytes, 706, 1037, 'RGB', 100, '4:4:4', output)!, `imagebytes-${bitpix}-3.jpg`, '', true)
			}
		}
	}
}, 5000)
