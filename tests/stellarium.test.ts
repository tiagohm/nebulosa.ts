...
test.skip('catalog', async () => {
	...
	expect(entries).toHaveLength(94660)
	...
	expect(entries[0].type).toBe(ObjectType.STAR)
	...
	expect(entries[311].type).toBe(ObjectType.HII_REGION)
	...
	expect(entries[94659].type).toBe(ObjectType.OPEN_STAR_CLUSTER)
