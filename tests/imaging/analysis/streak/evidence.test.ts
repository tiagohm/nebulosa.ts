...
describe('FieldCoherenceStreakEvidence', () => {
	...
	test('trusts an aligned tracking snapshot and does not fall through to stars', () => {
		...
		expect(voted[0]).toMatchObject({ class: 'trackingFailure', score: 0.9, weight: 0.8, tier: 'primary' })
		expect(voted[0]?.evidence[0]?.kind).toBe('trackingField')
		expect(provider.evaluate(aligned, { tracking: { ...snapshot, angle: undefined } })[0]).toMatchObject({ score: 0.9, tier: 'secondary', evidence: [{ description: 'field is coherently elongated, but the snapshot has no axis for this streak' }] })
		const unscaled = provider.evaluate(measured({ start: { x: 10, y: 40 }, end: { x: 210, y: 40 } }), { tracking: { ...snapshot, medianTrail: undefined } })
		expect(unscaled[0]).toMatchObject({ score: 0.9, weight: 0.8, tier: 'secondary', evidence: [{ kind: 'trackingField', description: 'field is coherently elongated, but the snapshot has no trail scale for this streak' }] })
		expect(provider.evaluate(measured({ start: { x: 10, y: 40 }, end: { x: 210, y: 40 } }), { tracking: { ...snapshot, medianTrail: 6 } })).toEqual([])
		expect(provider.evaluate(measured({ start: { x: 10, y: 40 }, end: { x: 18, y: 40 } }), { tracking: { ...snapshot, medianTrail: 6 } })[0]).toMatchObject({ score: 0.9, tier: 'primary' })
		expect(provider.evaluate(aligned, { tracking: { ...snapshot, score: 0.3 } })[0]).toMatchObject({ score: 0.3, tier: 'primary' })
