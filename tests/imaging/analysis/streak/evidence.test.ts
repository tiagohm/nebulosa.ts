...
describe('FieldCoherenceStreakEvidence', () => {
	...
	test('trusts an aligned tracking snapshot and does not fall through to stars', () => {
		...
		expect(provider.evaluate(aligned, { tracking: { ...snapshot, angle: undefined } })[0]).toMatchObject({ score: 1, tier: 'secondary', evidence: [{ description: 'field is coherently elongated, but the snapshot has no axis for this streak' }] })
		const unscaled = provider.evaluate(measured({ start: { x: 10, y: 40 }, end: { x: 210, y: 40 } }), { tracking: { ...snapshot, medianTrail: undefined } })
		expect(unscaled[0]).toMatchObject({ score: 1, weight: 0.8, tier: 'secondary', evidence: [{ kind: 'trackingField', description: 'field is coherently elongated, but the snapshot has no trail scale for this streak' }] })
