...
test('detects and refines the three synthetic spike orientations', () => {
	...
	const candidates = detectBahtinovHoughCandidates(preprocessed.ridgePoints, width, height, preprocessed.workspace, {
		maximumCandidates: 8,
		minimumAxialSeparation: Math.PI / 36,
		refinementRange: Math.PI / 180,
		refinementStep: Math.PI / 1800,
	})
	...
	for (const angle of expected) {
		...
		expect(best).toBeLessThan(Math.PI / 360)
	}
...
test('handles axial NMS across zero and PI', () => {
	...
	plotBahtinovSpikes(raw, width, height, 1, 40, 40, 100, 0, undefined, {
		normalAngles: [0.01, Math.PI / 2, Math.PI - 0.01],
		central: 1,
		halfLength: 28,
		taperLength: 4,
	})
	...
	expect(preprocessed.success).toBeTrue()
	if (!preprocessed.success) return
	const candidates = detectBahtinovHoughCandidates(preprocessed.ridgePoints, width, height, preprocessed.workspace, { minimumAxialSeparation: Math.PI / 18 })
	const boundaryCandidates = candidates.filter((candidate) => bahtinovAxialAngleDistance(candidate.normalAngle, 0) < Math.PI / 36)
