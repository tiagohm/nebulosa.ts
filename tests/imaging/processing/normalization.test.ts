...
describe('local normalization', () => {
	...
	test('alternate phases cover every residue pair for stride eight cells', () => {
		const size = 256
		const reference = referencePlane(20, size, size)
		const current = inverseTransform(
			reference,
			() => 1.2,
			(x, y) => 0.01 + 0.02 * (x / size) + 0.015 * (y / size),
	...
	test('dense retries supplement sparse cell samples instead of replacing them', () => {
		const size = 256
		const cellSize = 64
		const reference = new Float64Array(size * size)
		const current = new Float64Array(size * size)
