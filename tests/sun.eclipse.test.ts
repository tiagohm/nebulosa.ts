...
test('computeSolarEclipseMapGeometry anchors NASA total central endpoints', () => {
	...
	expect(lines.umbraNorth.map((line) => line.length)).toEqual([16, 5])
	expect(lines.umbraSouth.map((line) => line.length)).toEqual([17, 4])
	expect(geometry.polygons.totalityPath.map((ring) => ring.length)).toEqual([33, 9])
	expectGeoPointClose(lines.umbraNorth[0][0], -2.63628576222, -0.111228310133, 2460409.1952358116)
	expectGeoPointClose(lines.umbraNorth[0][8], -1.932980691808, 0.333802102165, 2460409.245936441)
	expectGeoPointClose(lines.umbraSouth[0][0], -2.733823414514, -0.14277306663, 2460409.1952358116)
	expectGeoPointClose(lines.umbraSouth[0][8], -1.928897575196, 0.304219635095, 2460409.245936441)
...
test('computeSolarEclipseMapGeometry keeps central path gated by eclipse gamma', () => {
	...
	expect(geometry.points.U2).toBeUndefined()
	expect(geometry.lines.centerLine).toHaveLength(0)
	expect(geometry.lines.umbraNorth.map((line) => line.length)).toEqual([16, 5])
	expect(geometry.lines.umbraSouth.map((line) => line.length)).toEqual([17, 4])
	expect(geometry.polygons.totalityPath.map((ring) => ring.length)).toEqual([33, 9])
	for (const segment of [...geometry.lines.umbraNorth, ...geometry.lines.umbraSouth, ...geometry.polygons.totalityPath]) for (const point of segment) expectGeoPoint(point)
})
...
test('computeSolarEclipseMapGeometry keeps umbral visibility for non-central total and annular eclipses', () => {
	...
	expectGeoPointClose(annular.lines.umbraNorth[0][0], 2.186520533702, -1.201642970219, 2456776.7524523144)
	expectGeoPointClose(annular.lines.umbraSouth[0][0], 2.261263031147, -1.242595100691, 2456776.7524523144)
	expectGeoPointClose(annular.polygons.totalityPath[0][0], 2.186520533702, -1.201642970219, 2456776.7524523144)
...
test('splitAtMaxAbsLatitude splits circumpolar-like limit arrays', () => {
	...
	expect(split[0].at(-1)!.latitude).toBe(deg(80))
	expect(split[1][0].latitude).toBe(deg(10))
})
