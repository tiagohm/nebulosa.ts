...
import { pixelScale } from '../src/formulas'
import { GuiderClient, type GuiderClientConnectOptions, type GuiderClientOptions } from '../src/guider.client'
import { writeImageToFits } from '../src/image'
import type { Image } from '../src/image.types'
import { type Camera, DEFAULT_CAMERA, DEFAULT_GUIDE_OUTPUT, type GuideDirection, type GuideOutput } from '../src/indi.device'
import type { CameraManager, DeviceHandler, GuideOutputManager } from '../src/indi.manager'
import { bufferSink } from '../src/io'
import type { PHD2Events } from '../src/phd2'
...
describe('construction', () => {
	...
	test('starts stopped, uncalibrated, unpaused and without a lock', () => {
		...
		expect(harness.client.getLockPosition()).toBeNull()
...
describe('frame-driven behavior', () => {
	...
	test('deselectStar drops the lock and returns to plain looping', async () => {
		...
		expect(harness.client.getLockPosition()).not.toBeNull()
