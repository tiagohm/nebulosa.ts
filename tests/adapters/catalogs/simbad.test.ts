...
test.skipIf(isNetworkTestSkipped())('query', async () => {
	const query = `
...
describe.skipIf(isNetworkTestSkipped())('simbad catalog', () => {
	...
	test('get by source id', async () => {
		...
		expect(formatRA(star!.rightAscension, false)).toBe('06 45 09')
		expect(formatDEC(star!.declination, false)).toBe('-16 42 58')
