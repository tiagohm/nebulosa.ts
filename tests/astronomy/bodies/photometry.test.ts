...
function sunToPlanet(body: (time: Time) => readonly [Vec3, Vec3]) {
	return vecMinus(body(NOW)[0], sun(NOW)[0])
}

// Earth -> planet vector at NOW.
function earthToPlanet(body: (time: Time) => readonly [Vec3, Vec3]) {
...
test('planetary visual magnitudes match the Mallama & Hilton 2018 reference', () => {
	...
	expect(planetMagnitude(Planet.MERCURY, sunToPlanet(mercury), earthToPlanet(mercury))).toBeCloseTo(1.8312, 3)
	expect(planetMagnitude(Planet.VENUS, sunToPlanet(venus), earthToPlanet(venus))).toBeCloseTo(-4.0593, 3)
	expect(planetMagnitude(Planet.MARS, sunToPlanet(mars), earthToPlanet(mars))).toBeCloseTo(1.3039, 3)
	expect(planetMagnitude(Planet.JUPITER, sunToPlanet(jupiter), earthToPlanet(jupiter))).toBeCloseTo(-1.8121, 3)
	expect(planetMagnitude(Planet.SATURN, sunToPlanet(saturn), earthToPlanet(saturn))).toBeCloseTo(0.7828, 3)
	expect(planetMagnitude(Planet.URANUS, sunToPlanet(uranus), earthToPlanet(uranus))).toBeCloseTo(5.8112, 3)
	expect(planetMagnitude(Planet.NEPTUNE, sunToPlanet(neptune), earthToPlanet(neptune), { year: YEAR })).toBeCloseTo(7.7647, 3)
})
...
test('the magnitude reduces to the distance modulus at zero phase', () => {
	...
	expect(planetMagnitude(Planet.JUPITER, sunToBody, observerToBody)).toBeCloseTo(-9.395 + 5 * Math.log10(20), 10)
})
...
test("Saturn's ringed model is undefined beyond a 6.5 deg phase angle", () => {
	...
	expect(planetMagnitude(Planet.SATURN, sunToBody, observerToBody)).toBeNaN()
	// The globe-alone variant stays defined.
	expect(Number.isNaN(planetMagnitude(Planet.SATURN, sunToBody, observerToBody, { rings: false }))).toBeFalse()
})
...
test('Neptune has no phase term before the year 2000', () => {
	...
	expect(planetMagnitude(Planet.NEPTUNE, s, e, { year: 1995 })).toBeNaN()
	expect(Number.isNaN(planetMagnitude(Planet.NEPTUNE, s, e, { year: 2026 }))).toBeFalse()
})
