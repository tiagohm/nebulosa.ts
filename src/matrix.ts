...
export class Matrix {
	...
	// Rotates the matrix 3x3 around the X axis by the given angle.
	rotX(angle: Angle, o?: Matrix) {
		o ??= this

		if (o.rows !== 3 || o.cols !== 3) {
			throw new Error('matrix must be 3x3 to rotate around X axis')
		}
		...
		const d = ca * o.get(1, 0) + sa * o.get(2, 0)
		const e = ca * o.get(1, 1) + sa * o.get(2, 1)
		const f = ca * o.get(1, 2) + sa * o.get(2, 2)
		const g = -sa * o.get(1, 0) + ca * o.get(2, 0)
		const h = -sa * o.get(1, 1) + ca * o.get(2, 1)
		const i = -sa * o.get(1, 2) + ca * o.get(2, 2)
	...
	// Rotates the matrix 3x3 around the Y axis by the given angle.
	rotY(angle: Angle, o?: Matrix) {
		o ??= this

		if (o.rows !== 3 || o.cols !== 3) {
			throw new Error('matrix must be 3x3 to rotate around Y axis')
		}
		...
		const a = ca * o.get(0, 0) - sa * o.get(2, 0)
		const b = ca * o.get(0, 1) - sa * o.get(2, 1)
		const c = ca * o.get(0, 2) - sa * o.get(2, 2)
		const g = sa * o.get(0, 0) + ca * o.get(2, 0)
		const h = sa * o.get(0, 1) + ca * o.get(2, 1)
		const i = sa * o.get(0, 2) + ca * o.get(2, 2)
	...
	// Rotates the matrix 3x3 around the Z axis by the given angle.
	rotZ(angle: Angle, o?: Matrix) {
		o ??= this

		if (o.rows !== 3 || o.cols !== 3) {
			throw new Error('matrix must be 3x3 to rotate around Z axis')
		}
		...
		const a = ca * o.get(0, 0) + sa * o.get(1, 0)
		const b = ca * o.get(0, 1) + sa * o.get(1, 1)
		const c = ca * o.get(0, 2) + sa * o.get(1, 2)
		const d = -sa * o.get(0, 0) + ca * o.get(1, 0)
		const e = -sa * o.get(0, 1) + ca * o.get(1, 1)
		const f = -sa * o.get(0, 2) + ca * o.get(1, 2)
	...
	static rotated(size: 3, angle: Angle, axis: 'x' | 'y' | 'z') {
		const ca = Math.cos(angle)
		const sa = Math.sin(angle)

		if (axis === 'x') return new Matrix(size, size, [1, 0, 0, 0, ca, sa, 0, -sa, ca])
		if (axis === 'y') return new Matrix(size, size, [ca, 0, -sa, 0, 1, 0, sa, 0, ca])
		else return new Matrix(size, size, [ca, sa, 0, -sa, ca, 0, 0, 0, 1])
	}

	// Creates a row matrix from an array.
	static row(data: NumberArray) {
		return new Matrix(1, data.length, data)
	}

	// Creates a column matrix from an array.
	static column(data: NumberArray) {
