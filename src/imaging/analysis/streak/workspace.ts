...
export interface StreakDetectionWorkspaceState {
	...
	// Edge/orientation-bin visits charged to coarse Hough voting in the latest call.
	houghCoarseEdgeWork: number
	// Edge/orientation-bin visits charged to angular Hough refinement in the latest call.
	houghRefinementEdgeWork: number
...
export function createStreakDetectionWorkspace(width: number, height: number, options: Readonly<StreakDetectionWorkspaceOptions> = {}): StreakDetectionWorkspace {
	...
	return {
		...
		state: { edgeCount: 0, candidateCount: 0, edgesTruncated: false, houghCoarseEdgeWork: 0, houghRefinementEdgeWork: 0, houghActiveAngles: 0, houghRhoWork: 0, refinementWork: 0, supportedRuns: 0, mergeRefits: 0 },
	}
}
