...
import type { SensorFrameSet, SensorPlane, SensorTileOptions } from './types'
...
export function measureSensorDarkCurrent(darks: readonly SensorFrameSet[], conversionGain: number, options: Partial<SensorDarkCurrentOptions> = {}): SensorDarkCurrent {
	...
	if (!Number.isFinite(meanRegression.slope)) throw new RangeError('dark mean slope must be finite')
