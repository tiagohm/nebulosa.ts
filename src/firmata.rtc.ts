...
export class DS3231 extends PeripheralBase<DS3231> implements RealTimeClock {
	...
	sync(date: Date = new Date()) {
		this.update(date.getFullYear(), date.getMonth() + 1, date.getDate(), date.getDay() + 1, date.getHours(), date.getMinutes(), date.getSeconds(), date.getMilliseconds())
	}
	...
	// Decodes the DS3231 hour register in either 12-hour or 24-hour mode.
