...
export class Wcs implements Disposable {
	...
	load(header: FitsHeader) {
		...
		const ret = wcspih(buffer, n, 0x000fffff, 0, nreject, nwcs, wcsprm)
	...
	pixToSky(x: number, y: number): [Angle, Angle] | undefined {
		if (this.pointer) {
			...
			const ret = wcsp2s(this.pointer, 1, 2, pixcrd, imgcrd, phi, theta, world, stat)
	...
	skyToPix(ra: Angle, dec: Angle): [number, number] | undefined {
		if (this.pointer) {
			...
			const ret = wcss2p(this.pointer, 1, 2, world, phi, theta, imgcrd, pixcrd, stat)
