...
// relation's exponential overflow to Infinity. The advertised domain is ±100 °C - the same plausible
// range the weather layer uses for these two sensors, and far wider than the -45..60 °C the
// approximation is actually fitted for - which caps the exponent near 17.5. Callers outside it get
// whatever the formula produces, which may be non-finite; isMagnusDomain tests the window.
...
export function eyepieceTrueFovViaFieldStop(fieldStopDiameterMm: number, telescopeFocalLengthMm: number) {
	return (RAD2DEG * fieldStopDiameterMm) / telescopeFocalLengthMm
}
...
export function plateScale(telescopeFocalLengthMm: number) {
	return ARCSEC_PER_RADIAN / telescopeFocalLengthMm
}
...
export function pixelScale(pixelSizeMicrons: number, telescopeFocalLengthMm: number) {
	return (ARCSECONDS_PER_PIXEL_FACTOR * pixelSizeMicrons) / telescopeFocalLengthMm
}
...
export function recommendedFocalLength(pixelSizeMicrons: number, targetSampling: number, seeingArcsec: number) {
	return (ARCSECONDS_PER_PIXEL_FACTOR * pixelSizeMicrons * targetSampling) / seeingArcsec
}
...
export function airyDiskSize(wavelengthMicrons: number, focalRatio: number) {
	return 2.44 * wavelengthMicrons * focalRatio
}
...
export function airyDiskInPixels(airyDiameterMicrons: number, pixelSizeMicrons: number) {
	return airyDiameterMicrons / pixelSizeMicrons
}
...
export function criticalFocusZone(wavelengthMicrons: number, focalRatioN: number) {
	return 4.88 * wavelengthMicrons * focalRatioN * focalRatioN
}
...
export function sensorDiagonalFov(sensorDiagonal: number, focalLength: number) {
	return 2 * Math.atan(sensorDiagonal / (2 * focalLength))
}
...
export function sensorFieldOfView(sensorSizeMm: number, focalLengthMm: number) {
	return (sensorSizeMm / focalLengthMm) * RAD2DEG
}
...
export function eyepieceView(telescopeFocalLengthMm: number, apertureMm: number, eyepieceFocalLengthMm: number, apparentFieldOfViewDegrees: number): EyepieceView {
	...
	return { magnification: power, trueFieldOfViewDegrees: apparentFieldOfViewDegrees / power, exitPupilMm: apertureMm / power }
}
...
export function mosaicPanelCount(targetFov: number, cameraFov: number, overlap: number) {
	return Math.ceil(targetFov / (cameraFov * (1 - overlap)))
}
...
export function guidingErrorInPixels(rmsArcsec: number, imageScaleArcsecPerPixel: number) {
	return rmsArcsec / imageScaleArcsecPerPixel
}
...
export function periodicErrorInPixels(periodicErrorArcsec: number, imageScaleArcsecPerPixel: number) {
	return periodicErrorArcsec / imageScaleArcsecPerPixel
}
...
export function starTrailLength(declination: Angle, exposureSeconds: number, imageScaleArcsecPerPixel: number) {
	return (SIDEREAL_RATE * Math.cos(declination) * exposureSeconds) / imageScaleArcsecPerPixel
}
...
export function stackingMagnitudeGain(frameCount: number) {
	return 1.25 * Math.log10(frameCount)
}
...
export function dynamicRange(fullWellElectrons: number, readNoiseElectrons: number) {
	return fullWellElectrons / readNoiseElectrons
}
...
export function saturationTime(fullWellElectrons: number, signalRateElectronsPerSecond: number) {
	return fullWellElectrons / signalRateElectronsPerSecond
}
...
export function skyLimitedExposure(readNoiseElectrons: number, skyRateElectronsPerSecond: number) {
	return (10 * readNoiseElectrons * readNoiseElectrons) / skyRateElectronsPerSecond
}
...
export function totalIntegrationTime(frameCount: number, exposureTimeSeconds: number) {
	return frameCount * exposureTimeSeconds
}
...
export function subframeCount(totalTimeSeconds: number, subExposureSeconds: number) {
	return totalTimeSeconds / subExposureSeconds
}
...
// [-100, 100], and relativeHumidityPercent is within (0, 100]. Outside that temperature window the
// a*T/(b+T) term is unbounded and the result may be non-finite; use isMagnusDomain to test a reading
...
// isMagnusDomain first. The range is far wider than any atmospheric reading, so it never excludes a
// physically meaningful input.
// Returns: estimated relative humidity in percent, finite and strictly positive inside the domain,
// reaching exactly 100 at saturation, where the dew point equals the ambient temperature. A dew point
// above the ambient temperature is supersaturated air and returns above 100 rather than failing, which
// also keeps the function usable on a saturated reading whose dew point exceeds the temperature only by
// rounding. Callers bound by a 0..100 contract clamp.
export function relativeHumidity(temperatureCelsius: number, dewPointCelsius: number) {
	const alpha = (MAGNUS_A_WATER * dewPointCelsius) / (MAGNUS_B_CELSIUS + dewPointCelsius) - (MAGNUS_A_WATER * temperatureCelsius) / (MAGNUS_B_CELSIUS + temperatureCelsius)
...
export function altitudeAtTransit(latitude: Angle, declination: Angle) {
	return PIOVERTWO - Math.abs(latitude - declination)
}
...
// altitude at hour angle -H (rising, east of the meridian) and +H (setting, west). Returns undefined when the
...
export function objectAngularDiameter(objectDiameter: number, distance: number) {
	return 2 * Math.atan(objectDiameter / (2 * distance))
}
...
export function surfaceBrightness(magnitude: number, areaArcsecSquared: number) {
	return magnitude + 2.5 * Math.log10(areaArcsecSquared)
}
...
export function cometMagnitudeEstimate(absoluteMagnitudeH: number, delta: Distance, heliocentricDistance: Distance, activityCoefficientK: number) {
	return absoluteMagnitudeH + 5 * Math.log10(delta) + activityCoefficientK * Math.log10(heliocentricDistance)
}
