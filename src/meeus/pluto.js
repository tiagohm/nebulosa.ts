...
export function heliocentric (jde) {
  ...
  for (const i in t37) {
    const t = t37[i]
    const [sAlpha, cAlpha] = base.sincos((t.i * J + t.j * S + t.k * P) * Math.PI / 180)
    l += t.lA * sAlpha + t.lB * cAlpha
    b += t.bA * sAlpha + t.bB * cAlpha
    r += t.rA * sAlpha + t.rB * cAlpha
  }
...
export function astrometric (jde, earth) {
  ...
  const f = function (jde) {
    ...
    // (37.1) p. 264
    const x = range * cl * cb
    const y = range * (sl * cb * cEpsilon - sb * sEpsilon)
    const z = range * (sl * cb * sEpsilon + sb * cEpsilon)
    return { x, y, z }
  }
