// moonposition.js, Chapter 47, Position of the Moon.
/**
 ...
 * @returns {Coord}
 *  {number} lon - Geocentric longitude lambda, in radians.
 *  {number} lat - Geocentric latitude beta, in radians.
 *  {number} range - Distance delta between centers of the Earth and Moon, in km.
 */
export function position (jde) {
  ...
  let sigmar = 0.0
  let sigmab = -2235 * sin(l_) + 382 * sin(a3) + 175 * sin(a1 - f) +
    175 * sin(a1 + f) + 127 * sin(l_ - m_) - 115 * sin(l_ + m_)
  ta.forEach((r) => {
    const [sina, cosa] = base.sincos(d * r.d + m * r.m + m_ * r.m_ + f * r.f)
    switch (r.m) {
      case 0:
        sigmal += r.sigmal * sina
        sigmar += r.sigmar * cosa
        break
      case -1:
      case 1:
        sigmal += r.sigmal * sina * e
        sigmar += r.sigmar * cosa * e
        break
      case -2:
      case 2:
        sigmal += r.sigmal * sina * e2
        sigmar += r.sigmar * cosa * e2
        break
    }
  })
  ...
  const lon = base.pmod(l_, 2 * Math.PI) + sigmal * 1e-6 * D2R
  const lat = sigmab * 1e-6 * D2R
  const range = 385000.56 + sigmar * 1e-3
...
const ta = (function () {
  ...
  return ta.map((row) => {
    const o = {}
    const vals = ['d', 'm', 'm_', 'f', 'sigmal', 'sigmar']
    vals.forEach((D2R, i) => {
      o[D2R] = row[i]
    })
    return o
