// globe.js, Chapter 11, The Earth's Globe.
/**
...
export class Ellipsoid {
  ...
  /**
   * parallaxConstants computes parallax constants rho sin phi′ and rho cos phi′.
   *
   * Arguments are geographic latitude phi in radians and height h
   * in meters above the ellipsoid.
   *
   * @param {number} phi - geographic latitude in radians
   * @param {number} h - height in meters above the ellipsoid
   * @return {number[]} [rho sin phi′, rho cos phi] parallax constants [rhosPhi, rhocPhi]
   */
  parallaxConstants (phi, h) {
    const boa = 1 - this.flat
    const su = Math.sin(Math.atan(boa * Math.tan(phi)))
    const cu = Math.cos(Math.atan(boa * Math.tan(phi)))
    const s = Math.sin(phi)
    const c = Math.cos(phi)
    const hoa = h * 1e-3 / this.radius
    // (s, c float)
    const rhosPhi = su * boa + hoa * s
    const rhocPhi = cu + hoa * c
    return [rhosPhi, rhocPhi]
  }
  ...
  /**
   * radiusOfCurvature of meridian at latitude phi.
   *
   * Result unit is Km.
   *
   * @param {number} phi
  ...
  distance (c1, c2) {
    ...
    const [s2Lambda, c2Lambda] = sincos2((c1.lon - c2.lon) / 2)
 ...
 * latitude (phi - phi′) with given geographic latitude (phi).
 *
 * Units are radians.
 * @param {number} phi
 * @returns {number} difference in Deg
 */
export function geocentricLatitudeDifference (phi) {
  ...
  return (692.73 * Math.sin(2 * phi) - 1.16 * Math.sin(4 * phi)) * Math.PI / (180 * 3600)
}
...
export class Coord {
  /**
   * @param {number} lat - latitude (phi) in radians
   * @param {number} lon - longitude (psi, or L) in radians (measured positively westward)
   */
