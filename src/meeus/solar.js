// solar.js, Chapter 25, Solar Coordinates.
/**
 ...
 * variation deltaLambda on p. 168) is not implemented.  Results for example 25.b
 ...
 *
 * @param {Number} T - number of Julian centuries since J2000. See base.J2000Century.
 * @returns {Number} Result is in radians and is not normalized to the range 0..2pi.
 */
...
export function apparentLongitude (T) {
  const omega = node(T)
  const {lon, ano} = trueLongitude(T) // eslint-disable-line
  return lon - 0.00569 * Math.PI / 180 - 0.00478 * Math.PI / 180 * Math.sin(omega)
}
...
export function trueEquatorial (jde) {
  ...
  // (25.6, 25.7) p. 165
  const ra = Math.atan2(cEpsilon * ss, cs)
  const dec = sEpsilon * ss
...
export function apparentEquatorial (jde) {
  ...
  const lambda = apparentLongitude(T)
  const epsilon = nutation.meanObliquity(jde)
  const [sLambda, cLambda] = base.sincos(lambda)
  // (25.8) p. 165
  const [sEpsilon, cEpsilon] = base.sincos(epsilon + 0.00256 * Math.PI / 180 * Math.cos(node(T)))
  const ra = Math.atan2(cEpsilon * sLambda, cLambda)
  const dec = Math.asin(sEpsilon * sLambda)
  return new base.Coord(ra, dec)
}
...
export function trueVSOP87 (planet, jde) {
  ...
  const s = lon + Math.PI
  // FK5 correction.
  const lambdap = base.horner(base.J2000Century(jde),
    s, -1.397 * Math.PI / 180, -0.00031 * Math.PI / 180)
  const [sLambdap, cLambdap] = base.sincos(lambdap)
  const deltaBeta = 0.03916 / 3600 * Math.PI / 180 * (cLambdap - sLambdap)
  // (25.9) p. 166
...
export function apparentVSOP87 (planet, jde) {
  ...
  const deltaPsi = nutation.nutation(jde)[0]
...
export function apparentEquatorialVSOP87 (planet, jde) {
  // note: duplicate code from ApparentVSOP87 so we can keep deltaEpsilon.
  ...
  const { ra, dec } = new coord.Ecliptic(lambda, lat).toEquatorial(epsilon)
