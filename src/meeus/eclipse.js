// eclipse.js, Chapter 54, Eclipses.
/**
 ...
 * @private
 */
const g = function (k, jm, c1, c2) { // (k, jm, c1, c2 float64)  (eclipse bool, jdeMax, gamma, u, Mʹ float64)
  const ck = 1 / 1236.85
  const p = Math.PI / 180
  const T = k * ck
  ...
  const omega = base.horner(T, 124.7746 * p, -1.56375588 * p / ck,
    0.0020672 * p, 0.00000215 * p)
  const sOmega = Math.sin(omega)
  const F1 = F - 0.02665 * p * sOmega
  ...
  const gamma = (P * cF1 + Q * sF1) * (1 - 0.0048 * W)
  const u = 0.0059 +
    0.0046 * Math.cos(M) * E +
    -0.0182 * Math.cos(Mʹ) +
    0.0004 * Math.cos(2 * Mʹ) +
    -0.0005 * Math.cos(M + Mʹ)
  return [eclipse, jdeMax, gamma, u, Mʹ] // (eclipse bool, jdeMax, gamma, u, Mʹ float64)
}
 ...
 *
 * gamma is the distance from the eclipse shadow center to the Earth center
 * at time jdeMax.
 *
 ...
 * eclipse types.
 *
 * gamma, u, and p are in units of equatorial Earth radii.
 */
export function solar (year) { // (year float64)  (eclipseType int, central bool, jdeMax, gamma, u, p, mag float64)
  let eclipseType = TYPE.None
  let mag

  const [e, jdeMax, gamma, u, _] = g(snap(year, 0), moonphase.meanNew(year), -0.4075, 0.1721) // eslint-disable-line no-unused-vars

  const p = u + 0.5461
  if (!e) {
    return { type: eclipseType } // no eclipse
  }
  const aGamma = Math.abs(gamma)
  if (aGamma > 1.5433 + u) {
    return { type: eclipseType } // no eclipse
  }
  const central = aGamma < 0.9972 // eclipse center touches Earth

  if (!central) {
    eclipseType = TYPE.Partial // most common case
    if (aGamma < 1.026) { // umbral cone may touch earth
      if (aGamma < 0.9972 + Math.abs(u)) { // total or annular
    ...
    const omega = 0.00464 * Math.sqrt(1 - gamma * gamma)
    if (u < omega) {
  ...
  if (eclipseType === TYPE.Partial) {
    // (54.2) p. 382
    mag = (1.5433 + u - aGamma) / (0.5461 + 2 * u)
  }
 ...
 *
 * gamma is the distance from the eclipse shadow center to the moon center
 * at time jdeMax.
 *
 * sigma is the radius of the umbral cone in the plane of the Moon.
 *
 * rho is the radius of the penumbral cone.
 *
 * mag is eclipse magnitude.
 *
 * sd- return values are semidurations of the phases of the eclipse, in days.
 *
 * gamma, sigma, and rho are in units of equatorial Earth radii.
 */
export function lunar (year) { // (year float64)  (eclipseType int, jdeMax, gamma, rho, sigma, mag, sdTotal, sdPartial, sdPenumbral float64)
  let eclipseType = TYPE.None
  let mag
  let sdTotal
  let sdPartial
  ...
  const [e, jdeMax, gamma, u, Mʹ] = g(snap(year, 0.5),
  ...
  const aGamma = Math.abs(gamma)
  mag = (1.0128 - u - aGamma) / 0.545 // (54.3) p. 382
  ...
  if (mag > 1) {
    ...
    mag = (1.5573 + u - aGamma) / 0.545 // (54.4) p. 382
    if (mag < 0) {
      return { type: eclipseType } // no eclipse
    }
  ...
  switch (eclipseType) {
    case TYPE.Total: {
      sdTotal = Math.sqrt(t * t - gamma2) / n / 24
    }
    case TYPE.Umbral: {
      sdPartial = Math.sqrt(p * p - gamma2) / n / 24
    }
    default: {
      const h = 1.5573 + u
      sdPenumbral = Math.sqrt(h * h - gamma2) / n / 24
    }
  }
  /* eslint-enable */
