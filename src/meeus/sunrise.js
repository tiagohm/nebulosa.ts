// sunrise.js, Compute rise, noon, set of the Sun for an earth observer
/**
...
import * as eqtime from './eqtime.js'
import * as solar from './solar.js'
import * as julian from './julian.js'
import * as rise from './rise.js'
import { signedDms, deg, toTimeSec } from '../angle'

const stdh0 = {
  sunrise:          signedDms(true, 0, 50, 0),
  sunriseEnd:       signedDms(true, 0, 18, 0),
  twilight:         signedDms(true, 6, 0, 0),
  nauticalTwilight: signedDms(true, 12, 0, 0),
  night:            signedDms(true, 18, 0, 0),
  goldenHour:       signedDms(false, 6, 0, 0)
}
...
export class Sunrise {
  ...
  _calcNoon (jde) {
    const etime = toTimeSec(eqtime.eSmart(jde))
    const delta = toTimeSec(this.lon)
  ...
  _calcRiseOrSet (jde, h0, isSet) {
    ...
    if (isSet) ha = -ha
    const delta = toTimeSec(ha - this.lon)
    const time = 43200 /* noon */ - delta - etime // in seconds
    return time / 86400
  }
