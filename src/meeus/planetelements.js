// planetelements.js, Chapter 31, Elements of Planetary Orbits.
/**
 ...
 * @param {Number} [node] - longitude of ascending node, omega
 * @param {Number} [peri] - longitude of perihelion, ϖ (Meeus likes pi better)
 */
...
export function mean (p, jde, e) {
  ...
  e.node = base.horner(T, c.omega) * Math.PI / 180
...
export function node (p, jde) { // (p int, jde float64)  float64
  return base.horner(base.J2000Century(jde), cMean[p].omega) * Math.PI / 180
}
