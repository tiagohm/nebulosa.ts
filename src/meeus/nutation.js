// nutation.js, Chapter 22, Nutation and the Obliquity of the Ecliptic.
/**
...
import { signedDms } from '../angle'

// Nutation: Chapter 22, Nutation and the Obliquity of the Ecliptic.

/**
 * Nutation returns nutation in longitude (deltaPsi) and nutation in obliquity (deltaEpsilon)
 * for a given JDE.
 *
 * JDE = UT + deltaT, see package.
 *
 ...
 * @return {number[]} [deltaPsi, deltaEpsilon] - [longitude, obliquity] in radians
 */
export function nutation (jde) {
  ...
  for (let i = table22A.length - 1; i >= 0; i--) {
    ...
    const arg = row.d * D + row.m * M + row.n * N + row.f * F + row.omega * omega
    const [s, c] = base.sincos(arg)
    deltaPsi += s * (row.s0 + row.s1 * T)
    deltaEpsilon += c * (row.c0 + row.c1 * T)
  }
  deltaPsi *= 0.0001 / 3600 * (Math.PI / 180)
  deltaEpsilon *= 0.0001 / 3600 * (Math.PI / 180)
  return [deltaPsi, deltaEpsilon] // (deltaPsi, deltaEpsilon float)
}
/**
...
export function approxNutation (jde) {
  ...
  const deltaPsi = (-17.2 * sOmega - 1.32 * s2L - 0.23 * s2N + 0.21 * s2Omega) / 3600 * (Math.PI / 180)
  const deltaEpsilon = (9.2 * cOmega + 0.57 * c2L + 0.1 * c2N - 0.09 * c2Omega) / 3600 * (Math.PI / 180)
  return [deltaPsi, deltaEpsilon] // (deltaPsi, deltaEpsilon float)
}

/**
 * MeanObliquity returns mean obliquity (epsilon₀) following the IAU 1980
 * polynomial.
 *
 ...
 *
 * @param {number} jde - Julian ephemeris day
 * @return {number} mean obliquity (epsilon₀)
 */
export function meanObliquity (jde) {
  // (22.2) p. 147
  return base.horner(
    base.J2000Century(jde),
    signedDms(false, 23, 26, 21.448),
...
/**
 * MeanObliquityLaskar returns mean obliquity (epsilon₀) following the Laskar
 * 1986 polynomial.
 *
...
const table22A = (function () {
  const PROPS = 'd,m,n,f,omega,s0,s1,c0,c1'.split(',')
  const tab = [
