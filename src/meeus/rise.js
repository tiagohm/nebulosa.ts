// rise.js, Chapter 15, Rising, Transit, and Setting.
...
import { signedDms, deg } from '../angle'
 ...
 */
export const meanRefraction = signedDms(false, 0, 34, 0)
...
/**
 * Stdh0Lunar is the standard altitude of the Moon considering pi, the
 * Moon's horizontal parallax.
 * @param {number} pi - the Moon's horizontal parallax
 ...
 */
export const stdh0Lunar = (pi, refraction) => {
  return stdh0.lunar * pi - (refraction || meanRefraction)
}
 ...
 */
export function hourAngle (lat, h0, delta) {
  // approximate local hour angle
  const cosH = (sin(h0) - sin(lat) * sin(delta)) / (cos(lat) * cos(delta)) // (15.1) p. 102
  if (cosH < -1) {
 ...
 * @param {number} alpha - right ascension in radians
 * @param {number} th0 - sidereal.apparent0UT in seconds of day `[0...86400[`
 * @return {number} time of transit in seconds of day `[0, 86400[`
 */
function _mt (lon, alpha, th0) {
  // const mt = (((lon + alpha) * 180 / Math.PI - (th0 * 360 / 86400)) * 86400 / 360)
  const mt = (lon + alpha) * SECS_PER_DEGREE * 180 / Math.PI - th0
  return mt
}
 ...
 * @param {number} alpha - right ascension (radians)
 * @param {number} delta - declination (radians)
 * @return {RiseObj} Result units are seconds and are in the range [0,86400)
 * @throws Error
 */
export function approxTimes (p, h0, Th0, alpha, delta) {
  const H0 = hourAngle(p.lat, h0, delta) * SECS_PER_DEGREE * 180 / Math.PI // in degrees per day === seconds
  // approximate transit, rise, set times.
  // (15.2) p. 102.0
  const mt = _mt(p.lon, alpha, Th0)
  const rs = {}
 ...
 *
 * @param {GlobeCoord} p - is geographic coordinates of observer.
 * @param {number} deltaT - is delta T in seconds
 * @param {number} h0 - is "standard altitude" of the body in radians
 * @param {number} Th0 - is apparent sidereal time at 0h UT at Greenwich in seconds
 *        (range 0...86400) must be the time on the day of interest, in seconds.
 *        See sidereal.apparent0UT
 * @param {Array<number>} alpha3 - slices of three right ascensions
 * @param {Array<number>} delta3 - slices of three declinations.
 *        alpha3, delta3 must be values at 0h dynamical time for the day before, the day of,
 *        and the day after the day of interest.  Units are radians.
 *
 * @return {RiseObj} Result units are seconds and are in the range [0,86400)
 * @throws Error
 */
export function times (p, deltaT, h0, Th0, alpha3, delta3) { // (p globe.Coord, deltaT, h0, Th0 float64, alpha3, delta3 []float64)  (mRise, mTransit, mSet float64, err error)
  const rs = approxTimes(p, h0, Th0, alpha3[1], delta3[1])
  const d3Alpha = new interp.Len3(-SECS_PER_DAY, SECS_PER_DAY, alpha3)
  const d3Delta = new interp.Len3(-SECS_PER_DAY, SECS_PER_DAY, delta3)

  // adjust mTransit
  const ut = rs.transit + deltaT
  const alpha = d3Alpha.interpolateX(ut)
  const th0 = _th0(Th0, rs.transit)
  const H = -1 * _mt(p.lon, alpha, th0) // in secs // Hmeus = 0...360
  rs.transit -= H
  ...
  const adjustRS = function (m) {
    const ut = m + deltaT
    const alpha = d3Alpha.interpolateX(ut)
    const delta = d3Delta.interpolateX(ut)
    const th0 = _th0(Th0, m)
    const H = -1 * _mt(p.lon, alpha, th0)
    const Hrad = (H / SECS_PER_DEGREE) * D2R
    const h = asin(((sLat * sin(delta)) + (cLat * cos(delta) * cos(Hrad)))) // formula 13.6
    const deltam = (SECS_PER_DAY * (h - h0) / (cos(delta) * cLat * sin(Hrad) * 2 * Math.PI)) // formula p103 3
    return m + deltam
  }
...
export class PlanetRise {
  ...
  constructor (jd, lat, lon, earth, planet, opts) {
    ...
    this.jde = cal.toJDE()
    this.deltaT = julian.deltaTSeconds(cal.toYear())
    this.earth = earth
    this.planet = planet
  }
  ...
  times () {
    ...
    const Th0 = sidereal.apparent0UT(this.jd)
    const rs = times(
      { lat: this.lat, lon: this.lon }, this.deltaT, this.refraction,
