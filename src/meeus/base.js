// base.js, Functions and other definitions useful with multiple packages.
/**
...
export function limb (equ, appSun) {
  const alpha = equ.ra
  const delta = equ.dec
  const alpha0 = appSun.ra
  const delta0 = appSun.dec
  // Mentioned in ch 41, p. 283.  Formula (48.5) p. 346
  const sDelta = Math.sin(delta)
  const cDelta = Math.cos(delta)
  const sDelta0 = Math.sin(delta0)
  const cDelta0 = Math.cos(delta0)
  const sAlpha0Alpha = Math.sin(alpha0 - alpha)
  const cAlpha0Alpha = Math.cos(alpha0 - alpha)
  let chi = Math.atan2(cDelta0 * sAlpha0Alpha, (sDelta0 * cDelta - cDelta0 * sDelta * cAlpha0Alpha))
  if (chi < 0) {
    chi += 2 * Math.PI
  }
  return chi
 ...
 * @returns {Number[]} [sin(epsilon), cos(epsilon)]
 */
export function sincos (epsilon) {
