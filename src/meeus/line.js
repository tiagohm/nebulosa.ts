// line.js, Chapter 19, Bodies in Straight Line
/**
 ...
 *
 * @returns {Number[]} [psi, omega]
 *  {Number} psi - angle between great circles defined by three points.
 *  {Number} omega - error angle of three nearly co-linear points
 */
export function angleError (r1, d1, r2, d2, r3, d3) {
  ...
  const psi = Math.acos((l1 * l2 + m1 * m2 + n1 * n2) /
    (Math.sqrt(l1 * l1 + m1 * m1 + n1 * n1) * Math.sqrt(l2 * l2 + m2 * m2 + n2 * n2)))
  const omega = Math.asin((a2 * l3 + b2 * m3 + c2 * n3) /
