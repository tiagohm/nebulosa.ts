// planetposition.js, Chapter 32, Positions of the Planets.
/**
...
export class Planet {
  ...
  position2000 (jde) {
    ...
    const tau = T * 0.1
    const lon = base.pmod(sum(tau, this.series.L), 2 * Math.PI)
    const lat = sum(tau, this.series.B)
    const range = sum(tau, this.series.R)
...
export function toFK5 (lon, lat, jde) {
  ...
  const Lp = lon - deg((1.397 + 0.00031 * T) * T)
  const [sLp, cLp] = base.sincos(Lp)
  // (32.3) p. 219
  const L5 = lon + arcsec(-0.09033 + 0.03916 * (cLp + sLp) * Math.tan(lat))
  const B5 = lat + arcsec(0.03916 * (cLp - sLp))
