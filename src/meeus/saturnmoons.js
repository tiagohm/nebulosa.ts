// saturnmoons.js, Chapter 46, Positions of the Satellites of Saturn
/**
...
export function positions (jde, earth, saturn) {
  ...
  const [s, beta, R] = [sol.lon, sol.lat, sol.range]
  ...
  let lambda0 = Math.atan2(y, x)
  let beta0 = Math.atan(z / Math.hypot(x, y))
  let ecl = new coord.Ecliptic(lambda0, beta0)
...
function R4 (lambda, r, gamma, omega) {
...
Qs.prototype.mimas = function () {
  ...
  r.gamma = 1.563 * d
  r.omega = 54.5 * d - 365.072 * d * this.t2
  return r
}
...
Qs.prototype.enceladus = function () {
  ...
  r.gamma = 0.0262 * d
  r.omega = 348 * d - 151.95 * d * this.t2
  return r
}

Qs.prototype.tethys = function () {
  const r = new R4()
  r.lambda = 285.306 * d + 190.69791226 * d * this.t1 +
    2.063 * d * this.sW0 + 0.03409 * d * this.s3W0 + 0.001015 * d * this.s5W0
  r.r = 4.880998
  r.gamma = 1.0976 * d
  r.omega = 111.33 * d - 72.2441 * d * this.t2
  return r
}
...
Qs.prototype.rhea = function () {
  ...
  const lambdaʹ = 359.244 * d + 79.6900472 * d * this.t1 + 0.086754 * d * sN
  const i = 28.0362 * d + 0.346898 * d * cN + 0.0193 * d * this.cW3
  const omega = 168.8034 * d + 0.736936 * d * sN + 0.041 * d * this.sW3
  const a = 8.725924
  return this.subr(lambdaʹ, p, e, a, omega, i)
}

Qs.prototype.subr = function (lambdaʹ, p, e, a, omega, i) {
  const r = new R4()
  const M = lambdaʹ - p
  const e2 = e * e
  const e3 = e2 * e
  ...
  const g = omega - 168.8112 * d
  const [si, ci] = base.sincos(i)
  const [sg, cg] = base.sincos(g)
  const a1 = si * sg
  const a2 = this.c1 * si * cg - this.s1 * ci
  r.gamma = Math.asin(Math.hypot(a1, a2)) // return value
  const u = Math.atan2(a1, a2)
  r.omega = 168.8112 * d + u // return value (w)
  const h = this.c1 * si - this.s1 * ci * cg
  const psi = Math.atan2(this.s1 * sg, h)
  r.lambda = lambdaʹ + C + u - g - psi // return value
  return r
}
...
Qs.prototype.titan = function () {
  ...
  const omegaʹ = 168.66925 * d + 0.628808 * d * this.sW3
  const [siʹ, ciʹ] = base.sincos(iʹ)
  const [sOmegaʹW8, cOmegaʹW8] = base.sincos(omegaʹ - this.W8)
  const a1 = this.sW7 * sOmegaʹW8
  const a2 = this.cW7 * siʹ - this.sW7 * ciʹ * cOmegaʹW8
  const g0 = 102.8623 * d
  const psi = Math.atan2(a1, a2)
  const s = Math.hypot(a1, a2)
  let g = this.W4 - omegaʹ - psi
  let ϖ = 0
  ...
  const b1 = siʹ * sOmegaʹW8
  const b2 = this.cW7 * siʹ * cOmegaʹW8 - this.sW7 * ciʹ
  const theta = Math.atan2(b1, b2) + this.W8
  const [sq, cq] = base.sincos(qq)
  const e = eʹ + 0.002778797 * eʹ * cq
  const p = ϖ + 0.159215 * d * sq
  const u = 2 * this.W5 - 2 * theta + psi
  const [su, cu] = base.sincos(u)
  const h = 0.9375 * eʹ * eʹ * sq + 0.1875 * s * s * Math.sin(2 * (this.W5 - theta))
  const lambdaʹ = L - 0.254744 * d *
    (this.e1 * Math.sin(this.W6) + 0.75 * this.e1 * this.e1 * Math.sin(2 * this.W6) + h)
  const i = iʹ + 0.031843 * d * s * cu
  const omega = omegaʹ + 0.031843 * d * s * su / siʹ
  const a = 20.216193
  return this.subr(lambdaʹ, p, e, a, omega, i)
}

Qs.prototype.hyperion = function () {
  const eta = 92.39 * d + 0.5621071 * d * this.t6
  const zeta = 148.19 * d - 19.18 * d * this.t8
  const theta = 184.8 * d - 35.41 * d * this.t9
  const thetaʹ = theta - 7.5 * d
  const as = 176 * d + 12.22 * d * this.t8
  const bs = 8 * d + 24.44 * d * this.t8
  const cs = bs + 5 * d
  const ϖ = 69.898 * d - 18.67088 * d * this.t8
  const phi = 2 * (ϖ - this.W5)
  const chi = 94.9 * d - 2.292 * d * this.t8
  ...
  const e = 0.103458 - 0.004099 * cEta - 0.000167 * cZetapEta + 0.000235 * cZetamEta +
    0.02303 * cZeta - 0.00212 * c2Zeta + 0.000151 * c3Zeta + 0.00013 * cPhi
  const p = ϖ + 0.15648 * d * sChi - 0.4457 * d * sEta - 0.2657 * d * sZetapEta - 0.3573 * d * sZetamEta -
    12.872 * d * sZeta + 1.668 * d * s2Zeta - 0.2419 * d * s3Zeta - 0.07 * d * sPhi
  const lambdaʹ = 177.047 * d + 16.91993829 * d * this.t6 + 0.15648 * d * sChi + 9.142 * d * sEta +
    0.007 * d * Math.sin(2 * eta) - 0.014 * d * Math.sin(3 * eta) + 0.2275 * d * sZetapEta +
    0.2112 * d * sZetamEta - 0.26 * d * sZeta - 0.0098 * d * s2Zeta -
    0.013 * d * Math.sin(as) + 0.017 * d * Math.sin(bs) - 0.0303 * d * sPhi
  const i = 27.3347 * d + 0.6434886 * d * cChi + 0.315 * d * this.cW3 + 0.018 * d * Math.cos(theta) -
    0.018 * d * ccs
  const omega = 168.6812 * d + 1.40136 * d * cChi + 0.68599 * d * this.sW3 - 0.0392 * d * scs +
    0.0366 * d * Math.sin(thetaʹ)
  return this.subr(lambdaʹ, p, e, a, omega, i)
}
...
Qs.prototype.iapetus = function () {
  ...
  const psi = 4.367 * d - 0.195 * d * this.t7
  const theta = 146.819 * d - 3.198 * d * this.t7
  const Phi = 60.47 * d + 1.521 * d * this.t7
  const phi = 205.055 * d - 2.091 * d * this.t7
  const eʹ = 0.028298 + 0.001156 * this.t11
  const ϖ0 = 352.91 * d + 11.71 * d * this.t11
  const mu = 76.3852 * d + 4.53795125 * d * this.t10
  const iʹ = base.horner(this.t11, 18.4602 * d, -0.9518 * d, -0.072 * d, 0.0054 * d)
  const omegaʹ = base.horner(this.t11, 143.198 * d, -3.919 * d, 0.116 * d, 0.008 * d)
  const l = mu - ϖ0
  const g = ϖ0 - omegaʹ - psi
  const g1 = ϖ0 - omegaʹ - phi
  const ls = this.W5 - ϖʹ
  const gs = ϖʹ - theta
  const lT = L - this.W4
  ...
  const p = ϖ0 + w / eʹ
  const lambdaʹ = mu - 0.04299 * d * su2 - 0.00789 * d * su1 - 0.06312 * d * Math.sin(ls) -
    0.00295 * d * Math.sin(2 * ls) - 0.02231 * d * Math.sin(u5) + 0.0065 * d * su5Psi
  const i = iʹ + 0.04204 * d * cu5Psi + 0.00235 * d * c5 + 0.0036 * d * cu2Phi
  const wʹ = 0.04204 * d * su5Psi + 0.00235 * d * s5 + 0.00358 * d * su2Phi
  const omega = omegaʹ + wʹ / Math.sin(iʹ)
  return this.subr(lambdaʹ, p, e, a, omega, i)
}
