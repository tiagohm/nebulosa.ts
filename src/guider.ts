...
export class Guider {
	...
	private cadenceScale(frame: GuideFrame) {
		const ts = frame.timestampMs
		if (ts === undefined || this.state.lastTimestampMs === undefined) return 1
		const dt = Math.max(1, ts - this.state.lastTimestampMs)
		return clamp(dt / this.config.nominalCadenceMs, 0.5, 2)
	}
